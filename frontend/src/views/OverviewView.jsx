 ...
 *  - Todos los KPIs críticos en una sola pantalla
 *  - Estado de conectividad de todos los sensores
 *  - Alertas activas globales
 *  - Mini-gráficas sparkline por variable
 *  - Balance energético instantáneo
 *  - Ideal como pantalla de monitorización permanente
 */
...
// ── KPI card compacta para overview ──────────────────────────
const OverviewCard = ({ variable, current, previous, sparkPoints }) => {
  ...
  return (
    <div className="bg-white border border-slate-100 shadow-sm hover:shadow-md
      transition-all rounded-2xl p-4 flex flex-col gap-2">
      ...
      <div className="flex items-baseline gap-1">
        <span className={`text-2xl font-black tracking-tight ${variable.text}`}>
          {current ?? '—'}
        </span>
        <span className={`text-xs font-semibold ${variable.text} opacity-60`}>
          {variable.unit}
        </span>
      </div>
...
const OverviewView = ({ sensorId = 's1' }) => {
  const [snapshots,     setSnapshots]     = useState({});  // { sensorId: latestData }
  const [sparklines,    setSparklines]    = useState({});  // { sensorId: { variable: [values] } }
  ...
  // WS para el sensor activo
  const { readings: wsReadings, connected: wsConnected, lastUpdate: wsTs }
  ...
  const load = useCallback(async (manual = false) => {
    ...
    try {
      ...
      // Datos en paralelo para todos los sensores
      const [snapRes, connRes, histResults, statusRes, energyRes] = await Promise.all([
        ...
        // Sparklines: últimas 3h de cada variable para el sensor activo
        Promise.all(
          VARIABLES.map(v =>
            api.getSFAHistory(sensorId, v.key, 3).catch(() => null)
          )
        ),
        // Estado (alertas activas)
        api.getSFAStatus(sensorId).catch(() => null),
        // Energía hoy
        api.getEnergyDaily(sensorId, 1).catch(() => null),
      ]);

      // Snapshots por sensor
      const snaps = {};
  ...
  useEffect(() => {
    ...
    setSnapshots(prev => ({
      ...prev,
      [sensorId]: { ...(prev[sensorId] ?? {}), ...wsReadings },
    }));
    // Actualizar sparklines en tiempo real (añadir último punto)
    setSparklines(prev => {
      const next = { ...prev };
      Object.entries(wsReadings).forEach(([key, value]) => {
        if (next[key]) {
          next[key] = [...next[key].slice(-29), value];
        }
      });
      return next;
    });
  ...
  return (
    <div className="flex flex-col gap-6 w-full mx-auto p-4 md:p-6 text-slate-800
        ...
        <div className="flex items-center gap-5">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5">
              Resumen ejecutivo · Todos los sistemas
            </p>
          </div>
        </div>
      ...
      {connectivity.length > 0 && (
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
          ...
          <div className="flex flex-wrap gap-2">
            {connectivity.map(s => (
              <ConnectivityBadge key={s.sensor_id} sensor={s} />
            ))}
          </div>
          {/* Detalles último visto */}
          ...
          <div className="flex flex-col gap-2">
            ...
            {alerts.length > 3 && (
              <p className="text-xs text-rose-500 font-medium pl-1">
                +{alerts.length - 3} más en el panel de estado
              </p>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          ...
          <div className="w-full bg-slate-100 rounded-full h-4 overflow-hidden shadow-inner">
            <div
              className={`h-full rounded-full transition-all duration-1000 ${battColor}`}
              style={{ width: `${soc ?? 0}%` }}
            />
          </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
            ...
            {[
              ...
              <div key={item.label} className={`${item.bg} rounded-xl p-3`}>
                ...
                <p className={`text-lg font-black ${item.color}`}>
                  {item.value != null
                    ? `${item.value >= 0 ? '' : ''}${Number(item.value).toFixed(1)}`
                    : '—'}
          ...
          <div className="mt-3 flex items-center gap-2 text-xs text-slate-400">
            <Zap size={13} />
            <span>
              Generando ahora: <strong className={`${(current?.i_generada ?? 0) > 0 ? 'text-emerald-600' : 'text-slate-500'}`}>
      ...
      {/* MULTI-SENSOR si hay más de uno */}
      {allSensors.length > 1 && (
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-4">
            ...
            <table className="w-full text-sm text-left">
              <thead className="text-xs font-bold text-slate-400 uppercase tracking-wider border-b border-slate-100">
                <tr>
                  ...
                  {VARIABLES.map(v => (
                    <th key={v.key} className="pb-3 px-3 text-right whitespace-nowrap">
                      {v.label}
                    </th>
                  ))}
                </tr>
                      ...
                      <td className="py-3 pr-4">
                        <div className="flex items-center gap-2">
                          <span className={`w-2 h-2 rounded-full flex-shrink-0
                            ${conn?.connected ? 'bg-emerald-500 animate-pulse' : 'bg-slate-300'}`} />
