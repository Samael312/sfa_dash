...
import {
  ...
  TrendingDown, BarChart2, Activity,
...
const KpiCard = ({ label, value, unit, sub, icon, color, bg, trend }) => (
  <div className="bg-white border border-slate-100 shadow-sm hover:shadow-md transition-shadow flex flex-col rounded-2xl overflow-hidden">
    ...
    <div className="px-6 py-3 mt-auto border-t border-slate-50 bg-slate-50/30 flex items-center justify-between">
...
const EnergyView = ({ sensorId = 's1' }) => {
  ...
  // Se añade parámetro para controlar cancelaciones de hilos asíncronos anteriores
  const load = useCallback(async (manual = false, activeSignal = { cancelled: false }) => {
    ...
    try {
      ...
      if (activeSignal.cancelled) return;

      setEnergyDaily(dailyRes?.data ?? []);
      setBalance(balanceRes ?? null);
    } catch (err) {
      if (!activeSignal.cancelled) {
  ...
  // Manejo del ciclo de vida y cambios de filtros sin desmontar UI completa
  useEffect(() => {
    const activeSignal = { cancelled: false };
    load(false, activeSignal);
  ...
  const dailyChartData = useMemo(() => {
    ...
    return {
      labels,
      datasets: [
        { label: 'Generada (Ah)',  data: energyDaily.map(d => d.gen_ah),  backgroundColor: 'rgba(16,185,129,0.75)', borderColor: '#10B981', borderWidth: 1.5, borderRadius: 6 },
        { label: 'Consumida (Ah)', data: energyDaily.map(d => d.load_ah), backgroundColor: 'rgba(99,102,241,0.65)', borderColor: '#6366F1', borderWidth: 1.5, borderRadius: 6 },
        {
          label: 'Neto (Ah)',
  ...
  const barOptions = {
    ...
    scales: {
      ...
      y: { grid: { color: '#F1F5F9', borderDash: [4, 4] }, ticks: { font: { size: 11 }, color: '#94A3B8', callback: v => `${v} Ah` } },
    },
  };
  ...
  const lineOptions = {
    ...
    scales: {
      ...
      y: { min: 0, max: 1, grid: { color: '#F1F5F9', borderDash: [4, 4] }, ticks: { font: { size: 11 }, color: '#94A3B8', callback: v => `${v} A` } },
    },
  };
  ...
  return (
    <div className="flex flex-col gap-6 w-full mx-auto p-4 md:p-6 text-slate-800 font-sans animate-in fade-in duration-500">
      ...
      {/* HEADER */}
      <div className="bg-white p-5 lg:p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-5">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
          ...
          <button onClick={() => load(true)} disabled={isRefreshing || loading}
            className="flex items-center gap-2 px-5 py-2 bg-emerald-600 hover:bg-emerald-700 text-sm font-semibold text-white rounded-xl shadow-sm shadow-emerald-200 transition-colors disabled:opacity-50 h-[38px]">
      ...
      {/* RENDERIZADO CONDICIONAL INTERNO PARA CARGA (No destruye los botones superiores) */}
      {loading && !isRefreshing ? (
        <div className="flex flex-col items-center justify-center h-96 bg-slate-50/50 rounded-3xl border border-dashed border-slate-200">
          <Loader2 className="animate-spin text-indigo-500 mb-4" size={40} />
          <p className="text-sm text-slate-500 font-medium">Cargando métricas de energía...</p>
        </div>
      ) : (
        <>
          {kpis && (
          ...
          <div className="bg-white border border-slate-100 shadow-sm rounded-2xl overflow-hidden">
            ...
            <div className="p-6 h-[320px]">
              {dailyChartData ? <Bar data={dailyChartData} options={barOptions} /> : <div className="h-full flex items-center justify-center text-slate-400"><p className="text-sm">Sin datos</p></div>}
            </div>
          </div>
                ...
                <table className="w-full text-sm text-left">
                  <thead className="bg-slate-50/50 text-slate-500 font-semibold border-b border-slate-100">
                    <tr>
                      ...
                      <th className="px-6 py-3 text-right text-emerald-600">Generada (Ah)</th>
                      <th className="px-6 py-3 text-right text-indigo-600">Consumida (Ah)</th>
                      <th className="px-6 py-3 text-right">Neto (Ah)</th>
                    </tr>
                  </thead>
          ...
          <div className="group bg-white border border-slate-100 shadow-sm rounded-2xl overflow-hidden transition-shadow hover:shadow-md">
            <div className="px-6 py-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">Corriente generada vs consumida (agrupado: {balance?.interval ?? '—'})</p>
                </div>
              </div>
              <div className="flex bg-slate-100/80 p-1 rounded-xl border border-slate-200/50">
            ...
            <div className="p-6 h-[320px] relative">
              <div className="absolute top-4 right-8 flex items-center gap-2 z-10">
                <button onClick={() => balanceChartRef.current?.resetZoom()} className="p-2 text-slate-400 bg-white/80 backdrop-blur border border-slate-200 shadow-sm hover:text-indigo-600 rounded-lg transition-all opacity-0 group-hover:opacity-100"><ZoomOut size={16} /></button>
                <button onClick={() => exportBalanceCSV(balance?.points, sensorId, balanceHours)} className="p-2 text-slate-400 bg-white/80 backdrop-blur border border-slate-200 shadow-sm hover:text-emerald-600 rounded-lg transition-all opacity-0 group-hover:opacity-100"><Download size={16} /></button>
              </div>
              {balanceChartData ? <Line ref={balanceChartRef} data={balanceChartData} options={lineOptions} /> : <div className="h-full flex items-center justify-center text-slate-400"><p className="text-sm">Sin datos</p></div>}
            </div>
          </div>
        </>
      )}
