/**
 * SOChart.jsx  (v2 — Coulomb Counting)
 * -------------------------------------
 * Gráfica + panel de SOC calculado por el backend:
 *
 *   SOC (%) → Coulomb Counting + calibración OCV a las ~04:00 AM
 *   (método correcto para batería plomo-ácido 12V 7.2Ah)
 *
 *   El SOC NO se calcula en el frontend a partir del voltaje.
 *   El voltaje se muestra como referencia (eje Y fijo 10–16V).
 *
 * Endpoints usados:
 *   GET  /sfa/soc/current   → SOC almacenado + metadatos
 *   POST /sfa/soc/compute   → recalcular Coulomb counting
 *   POST /sfa/soc/calibrate → forzar calibración OCV
 */
...
import { Loader2, ZoomOut, Download, RefreshCw, CheckCircle2, AlertCircle } from 'lucide-react';
...
const SOCChart = ({ sensorId = 's1', hours = 24, title = 'Estado de carga (SOC)' }) => {
  ...
  const load = useCallback(async () => {
    setError(null);
    try {
      const [voltRes, socRes] = await Promise.all([
        api.getSFAHistory(sensorId, 'v_bateria', hours),
        api.getSocCurrent(sensorId),
      ]);
      setVoltagePoints(voltRes?.points ?? []);
      setSocState(socRes ?? null);
    } catch {
      setError('Error al cargar datos de batería.');
    } finally {
      setLoading(false);
    }
  }, [sensorId, hours]);
  ...
  const handleCompute = async () => {
    ...
    try {
      ...
      setCalibMsg({ type: 'err', text: 'Error al recalcular.' });
    } finally {
      setCalibrating(false);
      setTimeout(() => setCalibMsg(null), 6000);
    }
  };
  ...
  if (error) return (
    <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-200">
      <p className="text-red-500 text-sm flex items-center gap-2"><AlertCircle size={15} /> {error}</p>
    </div>
  );
    ...
    <div className="group bg-white p-5 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-all flex flex-col gap-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1">
          <button onClick={handleCompute} disabled={calibrating} className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all disabled:opacity-50" title="Recalcular SOC (Coulomb Counting)">
            <RefreshCw size={14} className={calibrating ? 'animate-spin' : ''} />
          </button>
          <button onClick={() => chartRef.current?.resetZoom()} className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all opacity-0 group-hover:opacity-100" title="Restablecer zoom">
            <ZoomOut size={14} />
          </button>
          <button onClick={() => exportCSV(voltagePoints, sensorId, hours)} className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all opacity-0 group-hover:opacity-100" title="Exportar CSV">
            <Download size={14} />
          </button>
        </div>
      </div>
      ...
      <div className="flex items-center gap-5">
        <div className="flex flex-col items-center min-w-[4.5rem]">
          <span className={`text-5xl font-black tracking-tight
            ${currentSOC == null ? 'text-slate-300' : currentSOC > 60 ? 'text-emerald-600' : currentSOC > 20 ? 'text-amber-500' : 'text-rose-600'}`}>
            {currentSOC != null ? `${currentSOC}%` : '—'}
          </span>
        ...
        <div className="flex-1 flex flex-col gap-1.5">
          <div className="w-full bg-slate-100 rounded-full h-4 overflow-hidden shadow-inner">
            <div
              className={`h-full rounded-full transition-all duration-1000 ${currentSOC == null ? 'bg-slate-200' : currentSOC > 60 ? 'bg-emerald-500' : currentSOC > 20 ? 'bg-amber-400' : 'bg-rose-500'}`}
              style={{ width: `${currentSOC ?? 0}%` }}
            />
          </div>
      ...
      {calibMsg && (
        <div className={`text-xs font-semibold px-4 py-2 rounded-xl flex items-center gap-2
          ${calibMsg.type === 'ok' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : calibMsg.type === 'warn' ? 'bg-amber-50 text-amber-700 border border-amber-200' : 'bg-rose-50 text-rose-700 border border-rose-200'}`}>
      ...
      <div className="bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 flex flex-col sm:flex-row sm:items-center gap-3">
        <div className="text-xs text-slate-500 flex-1">
          ...
          <p>Activa ~04:00 AM si i_gen {"<"} 0.1A e i_carga {"<"} 0.1A durante ≥30 min (batería en reposo).</p>
